...
contract CarbonNFT is ERC1155, Ownable, ReentrancyGuard {
    ...
    // ===================== Structs =====================
    struct ProjectData {
        string projectName;     // Max 100 chars
        string projectType;    // Max 50 chars
        string location;       // Max 50 chars
        uint256 carbonReduction; // In tons (18 decimals)
        string methodology;    // e.g., "Verra VCS"
    ...
    // ===================== Events =====================
    event VerifierUpdated(uint256 index, string name, address wallet);
        ...
        onlyEOA
    {
        ...
        if (!autoApproveEnabled) {
            ...
            require(approvals[_currentTokenId][v.wallet], "Not approved by verifier");
        }
